...
The architecture Data Foundation aims to support working at scale with automation and self-service. Our People Pillar aims to help execution at scale by developing connections at all levels of the organization, increasing transparency, and through a strong network speeding up alignment and the execution of shared decisions.
...
The Data Foundation enables architects to move away from opinion-based decisions to data-driven economic risk modeling to become more data-driven. Such frameworks can help architects to achieve the following (credit Gregor Hohpe):
...
Misalignment frequently happens in big organizations. The [Data Foundation](#data) can increase organizational alignment by creating transparency. The [People Pillar](#people) develops global structures that can help people before and after they make an architectural decision:
...
* The Data Foundation, if implemented with a high level of automation, allows for quick extensions and reconfigurations to provide data for any change of direction. Extending the platform with new data should be easy after acquisitions or mergers. And the ability to group data at multiple levels supported the what-if scenario analysis (e.g., the impact of different reorganization scenarios). It defined a baseline to track changes in the organization.
* A strong Data Foundation provides crucial connections and feedback on the daily reality of each part of the organization.
...
* Lastly, the structure of our architectural model enables a more sustainable architectural function. With an established [Data Foundation](#data) and a well-connected architect, the architecture can still benefit the organization even without a strong central team.
...
* *Reflecting on the value of the "Data Foundation" concept, how effectively is your organization tracking changes, supporting what-if scenario analysis, and defining baselines?*

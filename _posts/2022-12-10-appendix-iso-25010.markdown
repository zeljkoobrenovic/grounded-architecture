...
* **Organize System in Limited Number (e.g., 7±2) Balanced Components:** When a system has too many or too few components, or if components differ in size significanlty (e.g., you have 80% of code in one common component) it is considered more challenging to understand and maintain.
* **Couple Files Loosely:** Software products where more source code resides in files strongly coupled with other files are deemed harder to maintain.
* **Keep Components Loosely Coupled:** Hide as much as possible of files in a component from direct dependencies from other components.
...
* **Avoid Many Parameters in Unit Interfaces:** Large number of parameters in a unit are deemed more complicated to maintain.
...
* **Manage Users Securely:** Implement secure and automated process for user sign-up, blocking and removal, and management of user credentials.
...
* **Optimize Common Transactions:** Identify the most comman and critical transactions, and apply common, relatively easy-to-implement strategies for their optimization.
